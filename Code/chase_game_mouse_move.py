...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            # if event.type == pygame.MOUSEBUTTONUP and not moveCheck:
            #     move = True
            #     moveCheck = True
            # else:
            #     move = False
        ...
        if not GAMEOVER:
            ...
            WorldUpdate(DISPLAYSURF, PLAYER, move, ENEMY, obstacles, POWER_UP, \
...
def WorldUpdate(surf, player, move, enemy, obstacles, power_up, run_time, GLOBAL_SPEED_MODIFIER):
    for ob in obstacles:
        ob.update()
...
class player(circ):
    ...
    def update_dir(self):
        ...
        self.velocity = vector3D(mouseX, mouseY, 0) - self.position
        self.direction = self.velocity.unit_vector()
